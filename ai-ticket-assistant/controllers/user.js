...
export const updateUser = async (req, res) => {
  ...
  try {
    if (req.user?.role !== "admin") {
      return res.status(403).json({ error: "Forbidden" }); // ✅ fixed typo in key
    }
...
export const getUsers = async (req, res) => {
  try {
    if (req.user?.role !== "admin") { // ✅ added safe optional chaining
    ...
    res.status(500).json({ error: "Fetch failed", details: error.message }); // ✅ fixed error message
  }
};
